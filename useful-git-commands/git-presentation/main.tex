    ...
    \begin{columns}
        \column{0.52\textwidth}
            ...
            \begin{block}{Basic commands}
                \texttt{git branch myidea} \quad create\\[4pt]
                \texttt{git checkout myidea} \quad switch to it\\[4pt]
                \texttt{git checkout main} \quad go back\\[4pt]
                \texttt{git branch -d myidea} \quad delete
            \end{block}
            }
            \vspace{0.4em}
            \uncover<4->{
            \begin{tcolorbox}[colback=red!10, colframe=red, boxrule=1pt,
                              arc=2pt, boxsep=4pt]
                \centering\textit{Experiment freely. If it fails,\\
...
\begin{frame}{All of this. No internet.}
    ...
    \vspace{0.8em}
    \large
    \texttt{init} \quad \texttt{add} \quad \texttt{commit} \quad \texttt{status} \quad \texttt{diff} \quad \texttt{log} \quad \texttt{reflog} \quad \texttt{reset} \quad \texttt{branch}\\[1.2em]
    \normalsize
    \pause
...
\begin{frame}[fragile]{Connecting to a hub}
    \begin{columns}
        \column{0.52\textwidth}
            Before pushing, your local repository\\
            needs to know where the remote is.\\[0.6em]
            \begin{lstlisting}[language=bash]
git remote add origin \
  git@github.com:you/myproject.git
            \end{lstlisting}
            \vspace{0.4em}
            \pause
            \begin{itemize}
                \item \texttt{origin} is just a name --- an alias
                \item You can have multiple remotes
                \item Done \textbf{once} per repository
            \end{itemize}
        \column{0.44\textwidth}
            \pause
